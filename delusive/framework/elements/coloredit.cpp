...
bool color_button(const char* desc_id, const ImVec4& col)
{
    ...
    draw->fade_rect_filled(window->DrawList, rect.Min + ImVec2(2, 2), rect.Max - ImVec2(2, 2), draw->get_clr(col), 
        draw->get_clr({col.x - 0.2f, col.y - 0.2f, col.z - 0.2f, col.w}), fade_direction::vertically);
...
bool color_picker(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    // ── HEX FIELD ─────────────────────────────────────────────────────────────
    {
        const float hex_total_w = sv_picker_size
            + style.ItemInnerSpacing.x + bars_width
            + (alpha_bar ? style.ItemInnerSpacing.x + bars_width : 0.f);

        const float hex_y = picker_pos.y + sv_picker_size + style.ItemInnerSpacing.y;

        PushFont(var->font.tahoma); // <-- фиксит символы-иконки

        draw_list->AddText(
            var->font.tahoma,
            var->font.tahoma->FontSize,
            ImVec2(picker_pos.x + 5.f, hex_y + 3.f),
            draw->get_clr(clr->widgets.text), "#");

        SetCursorScreenPos(ImVec2(picker_pos.x + 16.f, hex_y));
        PushItemWidth(hex_total_w - 16.f);

        PushStyleColor(ImGuiCol_FrameBg, draw->get_clr(clr->window.background_two));
        PushStyleColor(ImGuiCol_Border, draw->get_clr(clr->widgets.stroke_two));
        PushStyleColor(ImGuiCol_Text, draw->get_clr(clr->widgets.text));

        char hex_buf[10];
        if (alpha_bar)
            ImFormatString(hex_buf, sizeof(hex_buf), "%02X%02X%02X%02X",
                IM_F32_TO_INT8_SAT(col[0]), IM_F32_TO_INT8_SAT(col[1]),
                IM_F32_TO_INT8_SAT(col[2]), IM_F32_TO_INT8_SAT(col[3]));
        else
            ImFormatString(hex_buf, sizeof(hex_buf), "%02X%02X%02X",
                IM_F32_TO_INT8_SAT(col[0]), IM_F32_TO_INT8_SAT(col[1]),
                IM_F32_TO_INT8_SAT(col[2]));

        if (InputText("##hex", hex_buf, sizeof(hex_buf),
            ImGuiInputTextFlags_CharsHexadecimal |
            ImGuiInputTextFlags_CharsUppercase |
            ImGuiInputTextFlags_AutoSelectAll))
        {
            int r = 0, g_c = 0, b = 0, a = 255;
            if (alpha_bar && sscanf(hex_buf, "%02X%02X%02X%02X", &r, &g_c, &b, &a) == 4)
            {
                col[0] = r / 255.f; col[1] = g_c / 255.f;
                col[2] = b / 255.f; col[3] = a / 255.f;
                value_changed = true;
            }
            else if (sscanf(hex_buf, "%02X%02X%02X", &r, &g_c, &b) == 3)
            {
                col[0] = r / 255.f; col[1] = g_c / 255.f;
                col[2] = b / 255.f;
                value_changed = true;
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (BeginPopup("picker"))
        {
            if (g.CurrentWindow->BeginCount == 1)
            {
                ...
                value_changed |= color_picker("##picker", col, picker_flags | ImGuiColorEditFlags_AlphaBar, &g.ColorPickerRef.x);
...
bool c_gui::label_color_edit(std::string_view label, float col[4], bool active_alpha)
{
    ...
    gui->set_cursor_pos(
        ImVec2(
            GetWindowWidth() - elements->widgets.color_size.x - GetStyle().WindowPadding.x,
            GetCursorPos().y
        )
    );

    return gui->color_edit(label, col, active_alpha);
}