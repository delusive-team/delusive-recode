...
void c_draw::rect(ImDrawList* draw_list, const ImVec2& p_min, const ImVec2& p_max, ImU32 col, float rounding, ImDrawFlags flags, float thickness)
{
    ...
    if (draw_list->Flags & ImDrawListFlags_AntiAliasedLines)
        ...
        draw_list->PathRect(p_min + ImVec2(0.50f, 0.50f), p_max - ImVec2(0.49f, 0.49f), rounding, flags);
...
{
    draw->rect(GetBackgroundDrawList(), GetWindowPos() - ImVec2(1, 1), GetWindowPos() + GetWindowSize() + ImVec2(1, 1), draw->get_clr({0, 0, 0, 0.5f}));
    draw->rect_filled(GetWindowDrawList(), GetWindowPos() + ImVec2(0, var->window.titlebar), GetWindowPos() + GetWindowSize(), draw->get_clr(clr->window.background_two));
    ...
    draw->text_outline(GetWindowDrawList(), var->font.tahoma, var->font.tahoma->FontSize, GetWindowPos() + ImVec2(7, 4), draw->get_clr(clr->accent), GetCurrentWindow()->Name);
}