...
bool c_gui::keybind(const char* label, int* key, int* mode)
{
    ...
    else if (*key != 0 || (mode != nullptr && *mode == 2)) {
    ...
    if (mode != nullptr)
    {
        ...
        if (popup_open)
        {
            ...
            if (BeginPopupEx(popup_id, ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_AlwaysUseWindowPadding))
            {
                ...
                if (gui->selectable_ex("Always", 2 == *mode, ImVec2(60, 15))) *mode = 2;
...
bool c_gui::label_keybind(const char* label, int* key, int* mode, const char* bind_name)
{
    if (bind_name != nullptr && key != nullptr)
    {
        auto it = std::find_if(registered_keybinds.begin(), registered_keybinds.end(),
            [key](const keybind_reg_t& b) { return b.key == key; });

        if (it == registered_keybinds.end()) {
            registered_keybinds.push_back({ bind_name, key, mode, nullptr, 0.f });
        }
        else {
            it->display_name = bind_name;
        }
    }
