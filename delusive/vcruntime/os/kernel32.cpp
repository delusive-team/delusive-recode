...
LPVOID VirtualAllocEx( HANDLE process, LPVOID address, SIZE_T size, DWORD allocation_type, DWORD protect )
{
    ...
    if ( process == proc::NtCurrentProcess())
    {
...
{
    return os::VirtualAllocEx(proc::NtCurrentProcess(), address, size, allocation, protect );
}
...
{
    return os::VirtualFreeEx(proc::NtCurrentProcess(), address, size, type );
}
...
std::uintptr_t RtlpHpAllocWithExceptionProtection = 0;
...
std::uintptr_t RtlpHpFreeWithExceptionProtection = 0;
...
{
    return proc::GetProcessHeap( );
}

HANDLE GetCurrentProcess( )
{
    return proc::NtCurrentProcess( );
}

DWORD GetCurrentProcessId( )
{
    return proc::NtCurrentProcessId( );
}

HANDLE GetCurrentThread( )
{
    return proc::NtCurrentThread( );
}

DWORD GetCurrentThreadId( )
{
    return proc::NtCurrentThreadId( );
}
...
{
    return os::VirtualQueryEx( os::GetCurrentProcess( ), address, buffer, size );
}
...
{
    return os::VirtualProtectEx( os::GetCurrentProcess( ), address, dwSize, protect, restore );
}
} // namespace os
