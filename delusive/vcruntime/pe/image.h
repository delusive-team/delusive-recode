...
public:
    ...
    auto section( hash_t name_hash )
    {
        return get_section_impl( name_hash );
    }

    auto section( const char* name )
    {
        return get_section_impl( RHASH( name ) );
    }

    auto size( )
    {
        return static_cast<size_t>( m_nt_headers->OptionalHeader.SizeOfImage );
    }

    auto data_dir( uint16_t directory )
    {
        return get_data_dir_impl( directory );
    }
    ...
    auto get_import( hash_t name_hash )
    {
        return get_import_impl( name_hash );
    }

    auto get_import( const char* name )
    {
        return get_import_impl( RHASH( name ) );
    }
